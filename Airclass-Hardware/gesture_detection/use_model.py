...
hands = mp_hands.Hands(
        ...
        max_num_hands=1,  # Changed to detect only 1 hand
    ...
    if landmarks:
        ...
        if current_time - last_gesture_time >= GESTURE_COOLDOWN:
            ...
            if ZOOM_CONFIG["active"]:
                ...
                # Normal mode - original gesture handling (all gestures now work with single hand)
                if gesture_text == "call":
                    ...
                    send_websocket("timeout")  # Now works with single hand
                elif gesture_text == "three_gun":
                    thumb_tip = landmarks[4]  # Thumb tip
                    index_tip = landmarks[8]  # Index finger tip
                    ...
                    send_websocket("take_picture")  # Now works with single hand
                elif gesture_text == "hand_heart":
                    send_websocket("heart")  # Now works with single hand
                elif gesture_text == "hand_heart2":
                    send_websocket("hand_heart2")  # Now works with single hand
...
print("All gestures now work with single hand detection")
...
try:
    while True:
        ...
        if results.multi_hand_landmarks:
            # Process the detected hand (only one hand will be detected now)
            primary_hand = results.multi_hand_landmarks[0]
            
            # Get hand classification for the detected hand
        ...
        # Handle gesture processing (removed both_hands_stable parameter)
        ...
        if DEBUG:
            ...
            cv2.putText(frame, f"Hands: {hands_count}/1", (30, 150), 
